...
# ── Cupom ─────────────────────────────────────────────────────────────────────
# O campo "valor" representa um percentual de desconto (0 a 100).
...
def alternar_status_cupom(id_cupom):
    ...
    conn = get_db_connection()
    cupom = conn.execute('SELECT ativo FROM cupom WHERE id_cupom=?', (id_cupom,)).fetchone()
    if cupom:
        novo_status = 0 if cupom['ativo'] else 1
        conn.execute('UPDATE cupom SET ativo=? WHERE id_cupom=?', (novo_status, id_cupom))
        conn.commit()
    conn.close()
